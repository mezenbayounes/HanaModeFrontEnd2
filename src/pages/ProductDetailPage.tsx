...
import { ShoppingCart, CheckCircle, XCircle, ArrowLeft, Plus, Minus } from 'lucide-react';
...
export default function ProductDetailPage() {
  ...
  <p className="text-lg text-gray-600 mb-6">{product.description}</p>
                  ...
                  {product.sizes.map(s => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative px-6 py-3 font-semibold transition-all duration-300 border-2 ${
                          isDisabled
              ...
              {/* COLOR SELECTION (filtered by size) */}
             {/* COLOR SELECTION */}
{/* Only show color selection if there are any colors at all */}
<div className="mb-8">
  {product.sizes && product.sizes.some(s => s.colors && s.colors.length > 0) ? (
    ...
    // Empty placeholder to keep the same space
    <div className="h-[76px]"></div> 
            ...
            <button
              ...
              className={`w-full py-4 font-bold text-lg flex items-center justify-center gap-3 transition-all duration-300 border-2 ${
                product.inStock
                  ? 'bg-white text-black border-gray-600 hover:bg-black hover:text-white hover:border-black hover:shadow-xl hover:scale-105'
