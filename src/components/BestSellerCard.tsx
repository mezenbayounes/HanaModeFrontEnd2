...
export default function BestSellerCard({ product, size = 'default' }: BestSellerCardProps) {
  const { t } = useTranslation();
   ...
   const sizeClasses = {
     ...
     default: {
       ...
       priceGap: 'gap-2',
       buttonPadding: 'px-2 py-1 md:px-6 md:py-3',
      ...
      <div className="relative aspect-[4/5] overflow-hidden bg-gray-100 group/image">
       ...
       <div className={classes.padding}>
         ...
         <div className={`flex items-baseline ${classes.priceGap}`}>
           <span className={`${classes.priceMain} font-bold text-gray-900`}>{displayPrice.toFixed(2)} <br></br>DNT</span>
           {hasDiscount && (
       ...
       <div className={`w-full ${classes.buttonMargin} mt-auto`}>
  <button
    className={`w-full flex items-center justify-center gap-2 ${classes.buttonPadding} ${classes.buttonText} font-semibold border-2 border-gray-900 text-gray-900 bg-transparent transition-all duration-300 hover:bg-gray-900 hover:text-white transform hover:scale-[1.02] active:scale-95`}
  >
