...
interface ForgotPasswordModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSwitchToReset?: (email: string) => void; // To switch to reset password modal
  onSwitchToLogin?: () => void; // To switch back to login modal
}
...
export default function ForgotPasswordModal({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // After 1.5 seconds, switch to reset password modal or navigate
      setTimeout(() => {
        if (onSwitchToReset) {
        ...
        >
          ...
          <div className="p-8">
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              >
                {loading ? t('common.loading') : message ? t('auth.otpSent') : t('auth.sendOTP')}
              </button>
